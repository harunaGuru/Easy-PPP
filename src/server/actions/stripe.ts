...
export async function createCheckoutSession(tier: PaidTierNames){
    ...
    if(subscription?.stripeCustomerId == null){
        ...
        if(url == null) throw new Error("incorrect credentials") 
        redirect(url)
    }else{
...
export async function createCancelSession(){
    ...
    const portalSession = await stripe.billingPortal.sessions.create({
        ...
        return_url: `${clientEnv.NEXT_PUBLIC_APP_URL}/dashboard/subscription`,
        flow_data:{
...
export async function getSubscriptionUpgradeSession(tier: PaidTierNames, subscription:{
...
async function getCheckoutSession(tier: PaidTierNames, user: User){
    const session = await stripe.checkout.sessions.create({
        ...
        mode: "subscription",
        success_url: `${clientEnv.NEXT_PUBLIC_APP_URL}/dashboard/subscription`,
        cancel_url: `${clientEnv.NEXT_PUBLIC_APP_URL}/dashboard/subscription`,
    })
    return session.url
}